...
def name_hint(keyword, hits, chapters):
    """풀네임으로 훑어서 놓치는 것을 잡는다 (2026-08-24 신설).

    EP123 브리프에서 `정다은`으로 훑어 EP104 한 건만 잡혔고, 실제로는
    지면이 `다은`을 쓰기 때문에 7개 회차 25회였다. 인물을 성까지 붙여
    검색하면 거의 항상 놓친다 — 지면은 이름만 쓰기 때문이다.

    3음절 한글어가 적게 잡히면 뒤 2음절로 다시 세어 보고 알린다.
    """
    if not re.fullmatch(r"[가-힣]{3}", keyword) or len(hits) > 3:
        return
    short = keyword[1:]
    n = sum(1 for _, path in chapters for p in paragraphs(path) if short in p)
    if n <= len(hits):
        return
    print()
    print("  ★★★ 이름만 쓴 지면이 더 많다 — `%s` %d회 (지금 `%s` %d회)"
          % (short, n, keyword, len(hits)))
    print("     인물은 `--all %s`로 다시 훑을 것 (production-pipeline §8-2)" % short)
...
def sweep(keyword, chapters, recent, show_all):
    ...
    if not hits:
        ...
        name_hint(keyword, hits, chapters)
        return
